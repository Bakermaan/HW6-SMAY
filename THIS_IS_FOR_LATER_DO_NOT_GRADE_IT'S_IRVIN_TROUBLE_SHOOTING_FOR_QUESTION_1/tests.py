...
class ResistorNetwork():
    ...
    def MakeResistor(self, N, Txt):
        ...
        while "resistor" not in txt:
            print("Debug: Line", N, ":", txt)  # Debugging print statement
            if "name" in txt:
                R.Name = txt.split('=')[1].strip()
                print("Debug: Resistor name:", R.Name)  # Debugging print statement
            if "resistance" in txt:
                R.Resistance = float(txt.split('=')[1].strip())
                print("Debug: Resistor resistance:", R.Resistance)  # Debugging print statement
            N += 1
            if N < len(Txt):
                txt = Txt[N].lower()
            else:
                print("Debug: Reached end of file")  # Debugging print statement
                break

        self.Resistors.append(R)  # append the resistor object to the list of resistors
        print("Debug: Final Line Number:", N)
        print("Debug: Final Txt[N]:", Txt[N] if N < len(Txt) else "No more lines")  # Debugging print statement
        return N
    ...
    def AnalyzeCircuit(self):
        ...
        i0 = [2.0, 6.0, 8.0, 12.4,
              6.4]  # Adjust the size of this list based on the actual number of currents you are solving for
    ...
    def GetKirchoffVals(self, i):
        ...
        self.GetResistorByName('ec').Current = i[1]  # I_2 in diagram
        self.GetResistorByName('ef').Current = i[4]  # I_5 in diagram
        # calculate net current into node c

        Node_e_Current = sum([i[4],-i[1], -i[3]])
    ...
    def DisplayKVL(self):
        """
        Display Kirchhoff's Voltage Law equations for each loop in the circuit.
        """
        for i, loop in enumerate(self.Loops):
            print(f"Loop {i+1} KVL equation:")
            for n in range(len(loop.Nodes)):
                if n == len(loop.Nodes) - 1:
                    name = loop.Nodes[0] + loop.Nodes[n]
                else:
                    name = loop.Nodes[n] + loop.Nodes[n + 1]
                if name in [resistor.Name for resistor in self.Resistors]:
                    resistor = next(resistor for resistor in self.Resistors if resistor.Name == name)
                    print(f"    - V_{name} = {resistor.Resistance} * I_{name}")
                elif name in [source.Name for source in self.VSources]:
                    source = next(source for source in self.VSources if source.Name == name)
                    print(f"    - V_{name} = {source.Voltage}")
            print()
    ...
    def GetElementDeltaV(self, name):
        ...
        for v in self.VSources:
            ...
            if name[::-1] == v.Name:
                return v.Voltage
        # If element not found, return 0 or any other appropriate default value
        return 0.0
...
class VoltageSource():
    # region constructor
    def __init__(self, V=16.0, name='ab'):
        """
    ...
    def __init__(self, V=32.0, name='hg'):
        """
        Define a voltage source with instance variables of self.Voltage = V, self.Name = name
        :param V: The voltage
        :param name: the name of voltage source.  The voltage source naming convention is to use the nodes such as 'ab'
...
def main():
    ...
    Net.BuildNetworkFromFile('re3.txt')  # Call the method to build the network from the text file
